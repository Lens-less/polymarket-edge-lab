...
INSTALL_ROOT="${INSTALL_ROOT:-/opt/poly-mm-v06}"
DATA_ROOT="${DATA_ROOT:-/var/lib/poly-mm-v06}"
SERVICE_USER="${SERVICE_USER:-polybotv06}"
SERVICE_GROUP="${SERVICE_GROUP:-polybotv06}"
RESEARCH_DIR="btc_5m_15m_relative_value_paper_v06_linux_2026-08-14"
CONFIG_PATH="${INSTALL_ROOT}/research/${RESEARCH_DIR}/SERVICE_CONFIG.json"
PREREG_PATH="${INSTALL_ROOT}/research/${RESEARCH_DIR}/PREREGISTRATION.json"
...
for line in text.splitlines():
    ...
    if stripped.startswith("pool ") or stripped.startswith("server "):
        updated.append(
            ...
            else f"# disabled-by-poly-mm-v06 {line}"
        )
    else:
        updated.append(line)
if not any(line.strip().startswith("server 169.254.169.123") for line in updated):
    updated.extend(("", "# poly-mm v0.6 frozen clock source", amazon_line))
path.write_text("\n".join(updated).rstrip() + "\n", encoding="utf-8")
PY
...
install -d -o "${SERVICE_USER}" -g "${SERVICE_GROUP}" -m 0755 \
  "${DATA_ROOT}" "${DATA_ROOT}/data" "${DATA_ROOT}/research" \
  "${DATA_ROOT}/monitor" "${DATA_ROOT}/monitor/history"
install -o root -g root -m 0644 \
  "${INSTALL_ROOT}/deploy/aws/paper_v06/polymm-btc-twap-paper-v06.service" \
  /etc/systemd/system/polymm-btc-twap-paper-v06.service
install -o root -g root -m 0644 \
  "${INSTALL_ROOT}/deploy/aws/paper_v06/polymm-btc-twap-paper-v06-health.service" \
  /etc/systemd/system/polymm-btc-twap-paper-v06-health.service
install -o root -g root -m 0644 \
  "${INSTALL_ROOT}/deploy/aws/paper_v06/polymm-btc-twap-paper-v06-health.timer" \
  /etc/systemd/system/polymm-btc-twap-paper-v06-health.timer
install -o root -g root -m 0755 \
  "${INSTALL_ROOT}/deploy/aws/paper_v06/polymm-btc-twap-paper-v06-healthcheck.sh" \
  /usr/local/bin/polymm-btc-twap-paper-v06-healthcheck
...
assert prereg["scope"]["settlement_regime"] == "chainlink_twap_60s_5m_and_60s_15m"
assert Path("${DEPLOYMENT_REVISION_PATH}").read_text().strip() == "${DEPLOY_REF}"
assert Path("${IMPLEMENTATION_REVISION_PATH}").read_text().strip() == prereg["repository_head"]
for binding in (prereg["strategy_spec"], prereg["regime_registry"]):
...
printf '%s\n' \
  ...
  "  systemctl enable polymm-btc-twap-paper-v06.service" \
  "  systemctl start polymm-btc-twap-paper-v06.service" \
  "  systemctl enable polymm-btc-twap-paper-v06-health.timer" \
  "  systemctl start polymm-btc-twap-paper-v06-health.timer"
