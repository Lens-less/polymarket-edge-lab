...
**部分已过期（2026-08-21）。** 本指南原本围绕 `run_mm.py`（`SmartMarketMaker`，
一个与本项目 BTC 5m/15m 结构对策略无关的通用做市机器人）编写，该代码已删除。
环境准备（Python 版本、依赖安装、`.env` 配置）部分仍然有效，但"运行机器人"
一节描述的入口点已不存在。真正的策略见 `../../CLAUDE.md` 与
`../../research/btc_5m_15m_edge_readiness_v08_2026-08-18/`，目前处于纸面
研究阶段，没有一键启动的入口。
